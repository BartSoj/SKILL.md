...
- **Rules** work for skills that define properties the output must satisfy (SPEC, DOMAIN, INTERFACES). The agent has freedom in how it produces the output, as long as the constraints hold. Rules are declarative: "every function must have a full signature", "every endpoint has an idempotency stance."
