...
4. **Contract and error registries** (auto-discovered, optional) — if `INTERFACES.md` or `ERRORS.md` exist at the repository root, read them and pass their content to the Contracts Reviewer for cross-checking. See the Contract-registry cross-check note in Phase 2.
...
| Contracts Reviewer | `references/contracts-reviewer.md` | API design, type safety, breaking changes, invariants, backward compatibility. When `INTERFACES.md` or `ERRORS.md` exist at the repository root, also cross-checks wire formats and error codes against those registries. |
