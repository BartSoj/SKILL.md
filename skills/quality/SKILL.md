...
QUALITY.md consolidates what older guidance split as OBSERVABILITY.md and PERFORMANCE_MODEL.md, because every downstream reader needs both together: `/operations` wires alert routing and runbooks; `/WORK_UNITS.md` assigns `METRIC-*` and `SLO-*` ownership to work units; every `/spec` declares the signals the unit emits and the budgets it satisfies; `/review` verifies the declarations; `/system-verify` checks end-to-end that signals actually flow. The defining discipline — and the commonest violation — is **every SLI has an SLO, every SLO has a burn-rate alert, every alert links to a runbook**: orphan signals rot into noise; orphan SLOs decay into aspirations; orphan alerts page humans with no script to follow.
